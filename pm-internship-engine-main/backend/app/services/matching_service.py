...
from app.ml.feature_engineering.constants import SECTOR_KEYWORDS, education_match_score, get_sector_keywords
...
class MatchingService:
    ...
    def _score_stage(
        ...
        for opp in opportunities:
            # Education fit using shared constant
            qual_compat, level_match = education_match_score(candidate.education, opp.eligibility_criteria)
            education_fit = max(qual_compat, level_match)
    ...
    def _sector_interest(self, candidate: CandidateProfile, opp: Opportunity) -> float:
        ...
        relevant_skills = get_sector_keywords(sector_lower)

        if not relevant_skills:
            return 0.5

        overlap = sum(1 for s in candidate_skills_lower if s in relevant_skills)
