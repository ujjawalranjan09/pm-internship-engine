...
class HeuristicWeights:
    ...
    def to_array(self) -> np.ndarray:
        """Convert weights to numpy array matching feature order in FeatureVector."""
        # FeatureVector order: exact_skill_overlap, weighted_skill_similarity,
        # qualification_compatibility, distance_mobility, sector_affinity,
        # profile_completeness, competitiveness_percentile, past_participation,
        # district_representation, education_level_match, stipend_alignment,
        # semantic_similarity, text_keyword_overlap
        return np.array(
            [
                self.skill_match,               # exact_skill_overlap
                self.skill_match,               # weighted_skill_similarity (use same weight)
                self.qualification_fit,         # qualification_compatibility
                self.location_preference,       # distance_mobility
                self.sector_interest,           # sector_affinity
                self.profile_readiness,         # profile_completeness
                0.0,                            # competitiveness_percentile (not in weights)
                self.historical_adjustment,     # past_participation
                0.0,                            # district_representation (not in weights)
                self.qualification_fit,         # education_level_match
                0.0,                            # stipend_alignment (not in weights)
                self.semantic_similarity,       # semantic_similarity
                self.skill_match,               # text_keyword_overlap (use skill_match)
            ],
            dtype=np.float64,
        )
    ...
    def normalize(self) -> HeuristicWeights:
        ...
        return HeuristicWeights(
            ...
            employer_preference=float(normalized[13]),
