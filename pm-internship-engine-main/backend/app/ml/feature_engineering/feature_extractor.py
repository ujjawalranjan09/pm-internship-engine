...
from app.ml.feature_engineering.constants import EDUCATION_LEVELS, SECTOR_KEYWORDS, education_match_score, get_sector_keywords
...
# _EDUCATION_LEVELS and _SECTOR_KEYWORDS are now in constants.py
...
def _education_match_score(
    ...
    Both are in [0, 1].
    """
    return education_match_score(candidate_education, eligibility_criteria)
...
def _sector_affinity_score(
    ...
    keywords = get_sector_keywords(sector_lower)
...
class FeatureExtractor:
    ...
    def extract(
        ...
        # ── Education features ────────────────────────────────────
        qual_compat, edu_level = education_match_score(cand_edu, elig)
