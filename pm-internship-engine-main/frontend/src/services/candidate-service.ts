...
import type { CandidateProfile, CandidateListItem, CandidateFilter } from "@/types/candidate";
...
export async function getCandidateProfile(): Promise<CandidateProfile> {
  try {
    return await apiGet<CandidateProfile>(API_ROUTES.CANDIDATES.PROFILE);
  } catch {
    return MOCK_PROFILE;
  }
}
...
export async function updateCandidateProfile(data: Partial<CandidateProfile>): Promise<CandidateProfile> {
  try {
    return await apiPut<CandidateProfile>(API_ROUTES.CANDIDATES.UPDATE, data);
  } catch {
    return { ...MOCK_PROFILE, ...data };
  }
}

export async function getCandidateProfileCompletion(): Promise<{ completion: number }> {
  try {
    return await apiGet<{ completion: number }>(API_ROUTES.CANDIDATES.COMPLETION);
  } catch {
    return { completion: 92 };
  }
}