...
const MOCK_RESULTS: AllocationResult[] = [
  {
    ...
    matchScore: 87,
    allocatedAt: "2025-01-10T07:15:00Z",
    status: "confirmed",
    location: "Bangalore",
    sector: "Technology",
    stipend: 15000,
  ...
  },
  {
    ...
    opportunityId: "o6",
    opportunityTitle: "Cloud Infrastructure Intern",
...
export async function getCycles(): Promise<AllocationCycle[]> {
  try {
    return await apiGet<AllocationCycle[]>(API_ROUTES.ALLOCATION.CYCLES);
  } catch {
    return MOCK_CYCLES;
  }
}
...
export async function getCycleDetail(id: string): Promise<AllocationCycle> {
  try {
    return await apiGet<AllocationCycle>(API_ROUTES.ALLOCATION.CYCLE_DETAIL(id));
  } catch {
...
export async function getCycleResults(cycleId: string): Promise<AllocationResult[]> {
  try {
    return await apiGet<AllocationResult[]>(API_ROUTES.ALLOCATION.RESULTS(cycleId));
  } catch {
    return MOCK_RESULTS;
  }
}
...
export async function triggerAllocation(payload?: { name?: string }): Promise<AllocationCycle> {
  try {
    return await apiPost<AllocationCycle>(API_ROUTES.ALLOCATION.TRIGGER, payload);
  } catch {
    return {
      ...MOCK_CYCLES[1],
      id: `cycle_${Date.now()}`,
