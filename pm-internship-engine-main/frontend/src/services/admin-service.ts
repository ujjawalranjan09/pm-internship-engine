...
export async function getPolicy(): Promise<PolicyConfig> {
  try {
    return await apiGet<PolicyConfig>(API_ROUTES.ADMIN.POLICY);
  } catch {
    return MOCK_POLICY;
  }
}
...
export async function updatePolicy(data: Partial<PolicyConfig>): Promise<PolicyConfig> {
  try {
    return await apiPut<PolicyConfig>(API_ROUTES.ADMIN.POLICY, data);
  } catch {
...
export async function getAuditLog(): Promise<AuditEntry[]> {
  try {
    return await apiGet<AuditEntry[]>(API_ROUTES.ADMIN.AUDIT);
  } catch {
    return MOCK_AUDIT;
  }
}
...
export async function getOverrides(): Promise<OverrideRequest[]> {
  try {
    return await apiGet<OverrideRequest[]>(API_ROUTES.ADMIN.OVERRIDES);
  } catch {
...
export async function approveOverride(id: string): Promise<OverrideRequest> {
  try {
    return await apiPost<OverrideRequest>(API_ROUTES.ADMIN.APPROVE_OVERRIDE(id));
  } catch {
...
export async function rejectOverride(id: string): Promise<OverrideRequest> {
  try {
    return await apiPost<OverrideRequest>(API_ROUTES.ADMIN.REJECT_OVERRIDE(id));
  } catch {
    const override = MOCK_OVERRIDES.find((o) => o.id === id);
...
export async function getAnalytics(): Promise<Record<string, unknown>> {
  try {
    return await apiGet<Record<string, unknown>>(API_ROUTES.ADMIN.ANALYTICS);
  } catch {
    return {
...
export async function getAdminFairness(): Promise<FairnessMetrics> {
  try {
    return await apiGet<FairnessMetrics>(API_ROUTES.ADMIN.FAIRNESS);
  } catch {
    return {
