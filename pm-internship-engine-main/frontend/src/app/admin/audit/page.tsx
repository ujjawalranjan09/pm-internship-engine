...
const ACTION_COLORS: Record<string, string> = {
  ...
  "allocation.completed": "bg-emerald-100 text-emerald-800",
  "policy.updated": "bg-purple-100 text-purple-800",
  "override.approved": "bg-emerald-100 text-emerald-800",
                ...
                <TableBody>
                  {paged.map((entry) => (
                    <TableRow key={entry.id}>
                      <TableCell className="text-sm text-muted-foreground whitespace-nowrap font-[var(--font-jetbrains-mono)]">
                        {formatDateTime(entry.timestamp)}
                      </TableCell>
                      <TableCell className="font-medium text-sm font-[var(--font-dm-sans)]">{entry.performedByName}</TableCell>
                      <TableCell>
                        <Badge className={cn("text-xs", ACTION_COLORS[entry.action] ?? "bg-gray-100 text-gray-800")} size="sm">
                          {entry.action}
                        </Badge>
                      </TableCell>
                      <TableCell className="hidden md:table-cell text-sm text-muted-foreground font-[var(--font-jetbrains-mono)]">
                        {entry.entityType} / {entry.entityId}
                      </TableCell>
                      <TableCell className="hidden lg:table-cell text-xs text-muted-foreground max-w-[200px] truncate font-[var(--font-jetbrains-mono)]">
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between p-4 border-t">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-[var(--font-jetbrains-mono)]">Page {page} of {totalPages}</span>
