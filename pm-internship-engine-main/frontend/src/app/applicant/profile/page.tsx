...
import { useCandidateProfile, useUpdateProfile } from "@/hooks/use-candidates";
  ...
  const onSubmit = (data: ProfileFormData) => {
    ...
    const profileData: Partial<CandidateProfile> = {
      ...
      education: data.education?.map((e) => ({
        ...
        endYear: e.endYear,
        cgpa: e.cgpa,
        isCurrent: e.endYear === undefined || e.endYear >= new Date().getFullYear(),
      })),
      skills: selectedSkills.map((name) => ({ name, proficiency: "intermediate" as const })),
    };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium font-[var(--font-dm-sans)]">Profile Completion</span>
            <span className="text-sm font-bold text-[var(--role-primary-600)] font-[var(--font-space-grotesk)] tabular-nums">
            ...
            <div key={step.id} className="flex items-center">
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-[var(--role-primary-500)] text-white shadow-sm"
                    : isCompleted
              ...
              >
                ...
                <span className="hidden sm:inline font-[var(--font-dm-sans)]">{step.label}</span>
              </button>
              {idx < steps.length - 1 && (
                <div className={cn("w-6 h-0.5 mx-1", isCompleted ? "bg-emerald-300" : "bg-muted")} />
              )}
            </div>
          );
        })}
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Card>
          <CardHeader>
            <CardTitle className="font-[var(--font-space-grotesk)]">{steps[currentStep - 1].label}</CardTitle>
          ...
          <CardContent className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                {profile?.education && profile.education.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-sm font-medium font-[var(--font-dm-sans)]">Added Education</p>
                    {profile.education.map((edu) => (
                      <div key={edu.id} className="flex items-center justify-between p-3 rounded-lg border">
                        <div>
                          <p className="text-sm font-medium font-[var(--font-dm-sans)]">{edu.degree} - {edu.fieldOfStudy}</p>
                          <p className="text-xs text-muted-foreground font-[var(--font-dm-sans)]">{edu.institution} ({edu.startYear} - {edu.endYear ?? "Present"})</p>
                        </div>
            ...
            {currentStep === 3 && (
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1.5 font-[var(--font-dm-sans)]">Search & Add Skills</label>
                ...
                <div>
                  <p className="text-sm font-medium mb-2 font-[var(--font-dm-sans)]">Selected Skills</p>
                  {selectedSkills.length === 0 ? (
                    <p className="text-sm text-muted-foreground font-[var(--font-dm-sans)]">No skills selected yet. Search and add skills above.</p>
                  ) : (
                ...
                <Separator />
                <div>
                  <p className="text-sm font-medium mb-2 font-[var(--font-dm-sans)]">Suggested Categories</p>
                  <div className="flex flex-wrap gap-2">
                    {SKILL_CATEGORIES.filter((c) => !selectedSkills.includes(c)).slice(0, 8).map((cat) => (
                      <button
                        ...
                        className="px-3 py-1.5 text-xs rounded-full border border-dashed text-muted-foreground hover:border-[var(--role-primary-400)] hover:text-[var(--role-primary-600)] transition-colors font-[var(--font-dm-sans)]"
                      >
            ...
            {currentStep === 4 && (
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2 font-[var(--font-dm-sans)]">Sector Interests</label>
                  <p className="text-xs text-muted-foreground mb-2 font-[var(--font-dm-sans)]">Select sectors you&apos;re interested in</p>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isSelected
                              ? "bg-[var(--role-primary-50)] border-[var(--role-primary-300)] text-[var(--role-primary-700)]"
                  ...
                  {errors.sectors && (
                    <p className="text-sm text-gov-error mt-1 font-[var(--font-dm-sans)]">{errors.sectors.message}</p>
                  )}
                </div>
                <Separator />
                <div>
                  <label className="block text-sm font-medium mb-2 font-[var(--font-dm-sans)]">Location Preferences</label>
                  <p className="text-xs text-muted-foreground mb-2 font-[var(--font-dm-sans)]">Preferred cities/locations for internship</p>
                  <div className="flex flex-wrap gap-2">
            ...
            {currentStep === 5 && (
              <div className="space-y-4">
                <div
                  className="border-2 border-dashed rounded-xl p-8 text-center hover:border-[var(--role-primary-400)] transition-colors cursor-pointer"
                ...
                >
                  ...
                  {resumeFile ? (
                    <div>
                      <p className="text-sm font-medium text-foreground font-[var(--font-dm-sans)]">{resumeFile.name}</p>
                    ...
                    <div>
                      <p className="text-sm font-medium font-[var(--font-dm-sans)]">Drag & drop your resume here</p>
                      <p className="text-xs text-muted-foreground mt-1 font-[var(--font-dm-sans)]">or click to browse (PDF, DOC, DOCX - max 5MB)</p>
                    </div>
                  )}
                </div>
                {profile?.resumeUrl && !resumeFile && (
                  <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-50 border border-emerald-200">
                    ...
                    <span className="text-sm text-emerald-700 font-[var(--font-dm-sans)]">Resume previously uploaded</span>
                  </div>
                )}
              </div>
            )}
            ...
            {currentStep === 6 && (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <p className="text-sm font-medium font-[var(--font-dm-sans)]">Declaration</p>
                  <label className="flex items-start gap-3 p-4 rounded-lg border cursor-pointer hover:bg-muted/50 transition-colors">
                    <input
                      type="checkbox"
                      checked={declaration}
                      onChange={(e) => setDeclaration(e.target.checked)}
                      className="mt-0.5 h-4 w-4 rounded border-gray-300 text-[var(--role-primary-600)] focus:ring-[var(--role-primary-500)]"
                    />
                    <span className="text-sm text-muted-foreground font-[var(--font-dm-sans)]">
