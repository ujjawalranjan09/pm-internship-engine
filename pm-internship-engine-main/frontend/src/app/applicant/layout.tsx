...
export default function ApplicantLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col role-candidate">
      {/* Top Navbar */}
        ...
        <div className="flex h-16 items-center justify-between px-4 lg:px-6">
          <div className="flex items-center gap-3">
            ...
            <Link href="/applicant" className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg gov-gradient-candidate flex items-center justify-center">
                <span className="text-white font-bold text-sm font-[var(--font-space-grotesk)]">PM</span>
              </div>
              <span className="hidden sm:block font-semibold text-emerald-700 text-sm font-[var(--font-space-grotesk)]">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 h-4 w-4 rounded-full bg-emerald-500 text-white text-[10px] font-bold flex items-center justify-center">
          ...
          <nav className="flex flex-col h-full py-4 px-3" aria-label="Applicant navigation">
            <div className="flex-1 space-y-1">
              {navLinks.map((link) => {
                ...
                const isActive =
                  pathname === link.href || (link.href !== "/applicant" && pathname.startsWith(link.href));
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "bg-emerald-50 text-emerald-700 border-l-2 border-emerald-500 shadow-sm"
