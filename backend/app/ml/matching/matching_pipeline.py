...
from app.ml.ranking.heuristic_scorer import HeuristicScorer, ScoredCandidate
from app.ml.ranking.ml_ranker import MLRanker
from app.ml.ranking.reranker import Reranker, RerankerConfig, RerankedCandidate
...
class PipelineConfig:
    ...
    # Stage 1: Eligibility
    eligibility_fn: Optional[Callable] = None  # custom filter function
...
class MatchingPipeline:
    ...
    def __init__(self, config: Optional[PipelineConfig] = None) -> None:
        ...
        self._ml_ranker: Optional[MLRanker] = None
    ...
    def run(
        ...
        logger.info(
            ...
            len(candidates), len(opportunities),
        )
        ...
        score_matrix, feature_matrix, candidate_ids, opportunity_ids = (
            self._stage_ranking(candidates, opportunities, retrieval_map)
        )

        # ── Stage 4: Fairness Re-ranking ──────────────────────────
        if self.config.use_reranker:
            score_matrix = self._stage_reranking(
                score_matrix, candidate_ids, opportunity_ids, candidates, district_counts,
            )

        # ── Stage 5: Constrained Optimization ─────────────────────
        opt_result = self._stage_optimization(
            score_matrix, candidate_ids, opportunity_ids,
            candidates, opportunities, constraints, blocked_pairs,
        )
        ...
        if self.config.generate_explanations:
            explanations = self._generate_explanations(
                opt_result, feature_matrix, candidate_ids, opportunity_ids,
            )
        ...
        logger.info(
            ...
            result.num_allocated, result.total_candidates, total_duration,
        )
    ...
    def _stage_eligibility(
        ...
        self._stage_metrics.append(PipelineStageResult(
        ...
        for i, cand in enumerate(candidates):
            ...
            for oid in retrieval_map.get(cid, opportunity_ids):
                ...
                if self.config.use_ml_ranker and self._ml_ranker and self._ml_ranker.is_trained:
                    ml_score = float(self._ml_ranker.predict(
                        fv.to_array().reshape(1, -1), [cid], oid
                    )[0].score) if self._ml_ranker else h_score
                    blended = (
                        self.config.heuristic_weight * h_score
    ...
    def _stage_reranking(
        ...
        candidates: List[Dict[str, Any]],
        district_counts: Optional[Dict[str, int]],
    ) -> np.ndarray:
    ...
    def _stage_optimization(
        ...
        logger.info(
            ...
            result.num_allocated, result.solver_status, result.total_score,
        )
        return result
