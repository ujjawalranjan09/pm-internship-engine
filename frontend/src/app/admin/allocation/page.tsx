...
export default function AllocationPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="border-saffron-200 bg-saffron-50">
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="font-semibold text-saffron-800">Allocation in progress...</span>
            </div>
      ...
      {latestCycle && (
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <StatsCard title="Total Candidates" value={latestCycle.totalCandidates} icon={<Users className="h-5 w-5" />} />
          <StatsCard title="Opportunities" value={latestCycle.totalOpportunities} icon={<Briefcase className="h-5 w-5" />} />
          <StatsCard title="Allocations Made" value={latestCycle.totalAllocations} icon={<CheckCircle className="h-5 w-5" />} />
          <StatsCard
            title="Fairness Index"
            value={latestCycle.fairnessMetrics.representationIndex}
            format="percentage"
      ...
      <Card>
        <CardHeader>
          <CardTitle>Allocation Cycles</CardTitle>
          <CardDescription>History of all allocation runs</CardDescription>
        </CardHeader>
                ...
                {cycles.map((cycle) => (
                  <TableRow key={cycle.id}>
                    ...
                    <TableCell className="hidden md:table-cell">
                      {cycle.totalCandidates > 0 ? formatNumber(cycle.totalCandidates) : "—"}
                    </TableCell>
                    <TableCell className="hidden md:table-cell">
      ...
      <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
        <DialogContent onClose={() => setShowConfirm(false)}>
          <DialogHeader>
            <DialogTitle>Run Allocation Cycle</DialogTitle>
            <DialogDescription>
