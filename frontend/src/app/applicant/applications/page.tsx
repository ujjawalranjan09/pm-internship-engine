                    ...
                    >
                      <TableCell>
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground md:hidden">{app.company}</p>
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="hidden sm:table-cell">
                        <span className="text-sm font-medium text-navy-600">{app.matchScore}%</span>
                      </TableCell>
                    ...
                    {app.timeline.map((event, idx) => (
                      <div key={idx} className="relative flex items-start gap-3">
                        ...
                        <div className="ml-2">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">{formatDate(event.date, "dd MMM yyyy, hh:mm a")}</p>
                        </div>
                      </div>
                    ))}
                  </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total", value: MOCK_APPLICATIONS.length, color: "text-foreground" },
          { label: "Pending", value: MOCK_APPLICATIONS.filter((a) => ["submitted", "under_review"].includes(a.status)).length, color: "text-yellow-600" },
          { label: "Shortlisted", value: MOCK_APPLICATIONS.filter((a) => a.status === "shortlisted").length, color: "text-purple-600" },
          { label: "Allocated", value: MOCK_APPLICATIONS.filter((a) => a.status === "allocated").length, color: "text-green-600" },
        ].map((stat) => (
          <Card key={stat.label}>
