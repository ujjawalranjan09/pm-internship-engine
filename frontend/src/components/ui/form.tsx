...
const FormMessage = forwardRef<HTMLParagraphElement, HTMLAttributes<HTMLParagraphElement> & { name?: string }>(
  ({ className, children, name, ...props }, ref) => {
    let errorContent = children;
    if (name) {
      try {
        const { error } = useFormField();
        errorContent = error?.message ? String(error.message) : children;
      } catch {
    ...
    return (
      <p ref={ref} className={cn("text-sm font-medium text-gov-error", className)} role="alert" {...props}>
        {errorContent}
      </p>
    );
  }
